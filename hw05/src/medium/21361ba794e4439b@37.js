...
function _showInfo(d3,output_data,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 1100;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(output_data);
...
function _enlargeNode(d3,output_data,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 1100;
...
function _scaleNode(d3,output_data,drag,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 1100;
...
function _output_data(FileAttachment){return(
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("showInfo")).define("showInfo", ["d3","output_data","drag","invalidation"], _showInfo);
  main.variable(observer()).define(["md"], _4);
  main.variable(observer("enlargeNode")).define("enlargeNode", ["d3","output_data","drag","invalidation"], _enlargeNode);
  main.variable(observer()).define(["md"], _6);
  main.variable(observer("scaleNode")).define("scaleNode", ["d3","output_data","drag","invalidation"], _scaleNode);
  main.variable(observer("output_data")).define("output_data", ["FileAttachment"], _output_data);
